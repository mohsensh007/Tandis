...
namespace TandisWebApp.Controllers
{
    ...
    public class SecurityController : Controller
    {
        ...
        // ========== تولید تصویر QR (توکنی - کوتاه و خوانا) ==========
        [AllowAnonymous]
        [HttpGet]
        ...
        public async Task<IActionResult> GetQrCode([FromQuery] short shiftID = 1)
        {
            ...
            {
                _logger.LogError(ex, "خطا در تولید QR Code");
                return Content($"Error: {ex.Message}", "text/plain");
            }
        }

        // ========== اسکن و ورود به باشگاه ==========
        [HttpPost]
        ...
        public async Task<IActionResult> ValidateAndEnter([FromBody] ValidateQrRequest req)
        {
            try
            {
                ...
                // ۱. اعتبارسنجی توکن QR
                var (isValid, message) = await _qrService.ValidateTokenAsync(req.QrData);
                if (!isValid)
                    return Ok(new { success = false, message });

                // ۲. اطلاعات عضو + ثبت‌نام فعال + تنظیمات رشته
                ...
                var todayLatin = now.DayOfWeek.ToString(); // Saturday, Sunday, ...
                ...
                short webUserID = 1;
                byte trafficStatus = 1; // 1 = ورود
                bool isManual = false;
                var entryDesc = "ورود با QR Code";
                var personName = memberInfo?.Name ?? "";
                long? sportMemberID = activeReg.SportMemberID;

                // EntryDate و EntryTime عمداً ست نمی‌شن → DEFAULT دیتابیس (MiladiToShamsi / GetThisTime)
                await _db.Database.ExecuteSqlInterpolatedAsync($@"
                    INSERT INTO ACC_Traffic 
                        (MemberID, SportMemberID, TrafficStatus, EntryDesc, ShiftID, UserID, PersonName, EntryDateTime, IsManual, BoxID)
                    VALUES 
                        ({memberID}, {sportMemberID}, {trafficStatus}, {entryDesc}, {shiftID}, {webUserID}, {personName}, {now}, {isManual}, {assignedBoxID})");
        ...
        public async Task<IActionResult> Exit()
        {
            try
            {
                ...
                var openTrafficID = await _db.ACC_Traffics
                    .Where(t => t.MemberID == memberID && t.ExitDate == null && t.ExitDateTime == null)
                    .OrderByDescending(t => t.TrafficID)
                ...
                // TrafficStatus = 0 یعنی خروج (قرارداد: Enter=1 / Exit=0)
                // ExitDate و ExitTime با توابع خود دیتابیس پر می‌شن (هماهنگ با دسکتاپ)
                await _db.Database.ExecuteSqlInterpolatedAsync($@"
                    UPDATE ACC_Traffic
                    SET TrafficStatus = 0,
