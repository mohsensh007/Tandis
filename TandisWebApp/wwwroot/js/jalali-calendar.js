// ============================
// تقویم شمسی - jalali-calendar.js
// ============================

// تعریف متغیرها با var ساده
...
function jalaliToGregorian(jy, jm, jd) {
    jy -= 979; var gy = (jy <= 0) ? 621 : 1600; jy += (jy <= 0) ? 0 : 979;
...
// تابع toFa مخصوص این فایل
function toFaJalali(n) {
...
var calendarReturnModal = null;
var calendarAllowPast = false;
...
function renderCalendar(allowPast) {
    ...
    // استفاده از toFaJalali به جای toFa
    document.getElementById('currentMonthLabel').textContent = FaMonths[calendarDate.m - 1] + ' ' + toFaJalali(calendarDate.y);
    ...
    for (var w = 0; w < 7; w++) {
        ...
        for (var d = 0; d < 7; d++) {
            ...
            if (cellIdx < startOffset || day > daysInMonth) {
                ...
                if (isPast) {
                    html += '<td class="cal-disabled">' + toFaJalali(day) + '</td>';
                } else {
                    html += '<td class="cal-active" onclick="pickDay(' + day + ')">' + toFaJalali(day) + '</td>';
                }
                day++;
            }
        }
...
function pickDay(day) {
    ...
    document.getElementById(calendarCallback).value = dateStr;

    var dpModal = bootstrap.Modal.getInstance(document.getElementById('datePickerModal'));
...
window.toFaJalali = toFaJalali;

console.log('📅 Jalali Calendar loaded');